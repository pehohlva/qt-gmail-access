...
bool DocMargin::event(QEvent * e) {
    ...
    int Mcursor = qMax(Cursor_2_X, Cursor_1_X);
    int Scursor = qMin(Cursor_2_X, Cursor_1_X);
    int diffcursor = Mcursor - Scursor;
    ...
    if (e->type() == QEvent::MouseMove ||
        ...
        if (ke) {
            ...
            //// qDebug() << "### Rsite " << Rsite << " cm:" << POINT_TO_CM(Rsite);
            ////qDebug() << "### limit " << limit_left << ":" << limit_right;
            ///qDebug() << "### MaximumCollisionAllowed " << MaximumCollisionAllowed << " diffcursor " << diffcursor;
...
void DocMargin::resizeEvent(QResizeEvent *event) {
