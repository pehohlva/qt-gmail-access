...
<xsl:stylesheet 
  ...
  <xsl:variable name="paraMarginDefaultBottom">2pt</xsl:variable>
  ...
  <xsl:template name="DisplayAnnotationScript">
      ...
      <xsl:comment> Generator at <xsl:value-of select="$convert_time"/>  </xsl:comment>
        <xsl:text>
...
if (msoBrowserCheck())
{
  ...
  <xsl:template match="w:footnote">
   ...
   <!--  image src html reference to id inline word/media -->
   ...
   <img src="IDFROM_RELS={.//a:blip/@r:embed[1]}">
     <xsl:attribute name="width">
  ...
  <xsl:template match="/w:document">
    ...
    <html>
      ...
      <head>
        ...
        <meta name="Generator" content="XSLT gen on {$convert_time}" />
