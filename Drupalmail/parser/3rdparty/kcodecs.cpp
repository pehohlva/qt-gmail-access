/*
   ...
   The encoding and decoding utilities in MCodecs with the exception of
...
QByteArray MCodecs::quotedPrintableEncode(const QByteArray& in, bool useCRLF) {
    QByteArray out;
    quotedPrintableEncode(in, out, useCRLF);
    return out;
}

void MCodecs::quotedPrintableEncode(const QByteArray& in, QByteArray& out, bool useCRLF) {
    out.resize(0);
    if (in.isEmpty())
        return;
...
void MCodecs::quotedPrintableDecode(const QByteArray& in, QByteArray& out) {
