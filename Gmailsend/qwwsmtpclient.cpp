...
// Revision: Peter Hohl <pehohlva@gmail.com>,    19.9.2013
// to enable port 465 smtp from smtp.gmail.com
...
struct SMTPCommand {
    ...
    enum Type {
        Connect, Disconnect, StartTLS, Authenticate, Mail, RawCommand
    };
    int id;
    Type type;
    QVariant data;
...
void QwwSmtpClientPrivate::onConnected() {
    ...
    qDebug() << "onConnected intern:" << __FUNCTION__;
...
void QwwSmtpClientPrivate::_q_readFromSocket() {
    while (socket->canReadLine()) {
        ...
        qDebug() << "Remote host socket say:" << line;
        qDebug() << "mid:" << mid;
        qDebug() << "last:" << last;
        ...
        if (mid) {
            ...
            SMTPCommand &cmd = commandqueue.head();
            qDebug() << "single line switch status:" << status;
            qDebug() << "single line switch stage:" << cmd.extra.toInt();
            switch (cmd.type) {
                    // trying to connect
                case SMTPCommand::Connect:
                {
                    qDebug() << "SMTPCommand::Connect on " << __LINE__;
                    int stage = cmd.extra.toInt();
                ...
                case SMTPCommand::Authenticate:
                {
                    ...
                    if (stage == 0 && status == 334) {
                        ...
                        // auth failed
                        qDebug() << "auth failed!!!!" << status;
                        errorString = rxlast.cap(2).trimmed();
                        setState(QwwSmtpClient::Disconnecting);
                        processNextCommand();
                    } else if (stage == 1 && status == 535) {
                        // auth failed
                        qDebug() << "auth failed!!!!" << status;
                        errorString = rxlast.cap(2).trimmed();
                        setState(QwwSmtpClient::Disconnecting);
                        processNextCommand();
                    }  else if (stage == 1 && status == 451) {
                        // auth failed
                        qDebug() << "auth time out failed!!!!" << status;
                        errorString = rxlast.cap(2).trimmed();
                        setState(QwwSmtpClient::Disconnecting);
                        processNextCommand();
                    }  else if (status == 235) {
...
void QwwSmtpClientPrivate::processNextCommand(bool ok) {
    ...
    qDebug() << "processNextCommand cmd.type:" << cmd.type;
    ...
    switch (cmd.type) {
        ...
        case SMTPCommand::Authenticate:
        {
            qDebug() << "SMTPCommand::Authenticate";
            QwwSmtpClient::AuthMode authmode = (QwwSmtpClient::AuthMode)cmd.data.toList().at(0).toInt();
            switch (authmode) {
                case QwwSmtpClient::AuthPlain:
        ...
        {
            /// responder ok
            const QByteArray comander_to_send = QString("MAIL FROM: <%1>\r\n").arg(cmd.data.toList().at(0).toString()).toUtf8();
            qDebug() << "send comand:" << comander_to_send;
            setState(QwwSmtpClient::Sending);
            socket->write(comander_to_send);
        }
            break;
        case SMTPCommand::RawCommand:
        {
            qDebug() << "SMTPCommand::RawCommand";
...
void QwwSmtpClientPrivate::parseOption(const QString &buffer) {
    if (buffer.toLower() == "pipelining") {
        options |= QwwSmtpClient::PipeliningOption;
    } else if (buffer.toLower() == "starttls") {
        options |= QwwSmtpClient::StartTlsOption;
    } else if (buffer.toLower() == "8bitmime") {
        options |= QwwSmtpClient::EightBitMimeOption;
    } else if (buffer.toLower().startsWith("auth ")) {
        options |= QwwSmtpClient::AuthOption;
        // parse auth modes
...
int QwwSmtpClient::connectToHostEncrypted(const QString & hostName, quint16 port) {
    qDebug() << "Play:" << __FUNCTION__;
    qDebug() << "go to host:" << hostName;
